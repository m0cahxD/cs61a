...
def make_tweet_fn(text, time, lat, lon):
    ...
    def tweet_fn(arg):
        if arg == 'text':
            ...
            print('Error! You know nothing John Snow!')
            assert False
    return tweet_fn
...
def analyze_tweet_sentiment(tweet):
    ...
    words = map(get_word_sentiment, extract_words(text))
    count = 0
    value = 0
    for word in words:
        if has_sentiment(word):
            count += 1
            value += sentiment_value(word)
    if count != 0:
        average = value/count
    else:
        average = None
...
def find_centroid(polygon):
    ...
    while count < len(polygon) - 1:
        area = area + (1 / 2)*((latitude(polygon[count]) * longitude(polygon[count + 1])) -  (latitude(polygon[count+ 1]) * longitude(polygon[count])))
        count += 1
    if area == 0:
        ...
        cy = 0
        while count < len(polygon) - 1:
            cx = cx + ((latitude(polygon[count]) + latitude(polygon[count + 1])) * ((latitude(polygon[count]) * longitude(polygon[count + 1])) -  (latitude(polygon[count+ 1]) * longitude(polygon[count]))))
            cy = cy + ((longitude(polygon[count]) + longitude(polygon[count + 1])) * ((latitude(polygon[count]) * longitude(polygon[count + 1])) -  (latitude(polygon[count+ 1]) * longitude(polygon[count]))))
            count += 1
        cx = cx / (6 * area)
        cy = cy / (6 * area)
        area = abs(area)
...
def average_sentiments(tweets_by_state):
    ...
    for x in tweets_by_state.keys():
        sentiment = 0
        check_if_all_none = True
        none_counter = 0
        for y in tweets_by_state[x]:
            if has_sentiment(analyze_tweet_sentiment(y)):
                none_counter += 1
                check_if_all_none = False
                sentiment = sentiment + sentiment_value(analyze_tweet_sentiment(y))
        if check_if_all_none == False:
            sentiment = sentiment / none_counter
            averaged_state_sentiments[x] = sentiment
