...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while (score < GOAL_SCORE and opponent_score < GOAL_SCORE):
    	if who == 0:
    		strategy = strategy0(score, opponent_score)
    		score += take_turn (strategy, opponent_score, select_dice(score, opponent_score))
    	else:
    		strategy = strategy1(opponent_score, score)
    		opponent_score += take_turn (strategy, score, select_dice(score, opponent_score))
    	who = other(who)
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    count = 0
    for x in range(1, 11):
        current_average = make_averaged(roll_dice)(x, dice)
        print("%s dice scores %s on average" %(x, current_average))
        if max_so_far < current_average:
            max_so_far = current_average
            count = x
    return count
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=5):
    ...
    if score + bacon == 2 * opponent_score:
        return num_rolls
    elif 2 * (score + bacon) == opponent_score:
        return 0
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    "*** YOUR CODE HERE ***"
    bacon = 1 + max(opponent_score//10, opponent_score%10)
    # uses swine swap when advantageous and avoids it when not
    if score + bacon == 2 * opponent_score:
        return 5
    if 2 * (score + bacon) == opponent_score:
        return 0

    # attempts to give opponents more 4 sided dice
    if (score + opponent_score + bacon) % 7 == 0:
        return 0

    # what to do when rolling a four sided dice
    if (score + opponent_score) % 7 == 0:
