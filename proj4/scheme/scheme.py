...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            # construct the scheme list into a python list of functions
            argument_list = []
            current = args
            while current != nil:
                argument_list.append(current.first)
                current = current.second

            if self.use_env:
                argument_list.append(env)

            value = self.fn(*argument_list)
            return (value, None)

        except TypeError:
            raise SchemeError("Q3")
...
class MuProcedure(LambdaProcedure):
    ...
    def apply(self, args, env):
        if proper_tail_recursion:
            ...
            print(self.formals)
            print(args)
            frame = Frame(env).make_call_frame(self.formals, args)
            value = scheme_eval(self.body, frame)
            return (value, None)
...
def check_formals(formals):
    ...
    checked = []
    while formals != nil:
        if scheme_symbolp(formals.first) and formals.first not in checked:
            checked.append(formals.first)
            formals = formals.second
        else:
            raise SchemeError("QB11")
