...
(define (list-partitions total max-pieces max-value)
  ...
  ;; Helper function to construct the partitions
  (define (helper total max-pieces max-value current)
    (cond ((= total 0) (list current))
          ((or (< total 0) (= 0 max-pieces) (= 0 max-value)) nil)
          (else 
            (append
            (helper (- total max-value) (- max-pieces 1) max-value (append current (list max-value)))
            (helper total max-pieces (- max-value 1) current)))
      )
    )
...
(define (tree-sums tree)
  ; *** YOUR CODE HERE ***
  (define (tree-helper input-tree sum)
    (print sum)
    (if (null? input-tree) sum
        (append
          (map  (lambda (x) (tree-helper x (append sum (list (entry input-tree)))))  (children input-tree))
          )
      )
    )
  (tree-helper tree ())
