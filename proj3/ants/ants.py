...
# Name: Kevin Hui & Vividly Vivian Dien
        ...
        if not insect.is_ant():
            ...
            if insect.container:
                ...
                if self.ant.name == 'Queen':
                    if self.ant.true_queen:
                        print("Can't remove the TRUE queen")
                    else:
                        self.ant = None
                        insect.place = None
                else:
...
class Insect:
    ...
    def __init__(self, armor, place=None):
        ...
        self.status = 0  # implemented for EC
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        contain = self.container and self.ant is None and not other.container
        return contain
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        nearest_enemy = self.place
        while count > 0:
            nearest_enemy = nearest_enemy.entrance
            count -= 1
        if nearest_enemy is None:
            return random_or_none([])
        count = self.max_range
        while len(nearest_enemy.bees) == 0 and count > 0:
            if nearest_enemy.entrance == None:
                break
            if nearest_enemy.entrance.name == 'Hive':
                break
            count -= 1
            nearest_enemy = nearest_enemy.entrance
        return random_or_none(nearest_enemy.bees)
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        "*** YOUR CODE HERE ***"
        if self.digesting <= 0:
            nom = random_or_none(self.place.bees)
            if nom is not None:
                self.eat_bee(nom)
            ...
            while place.entrance is not None:
                ...
                if place.ant is not None:
                    if not place.ant.buffed:
                        ...
                        place.ant.buffed = True
                        if place.ant.container:
                            if not place.ant.ant.buffed:
                                place.ant.ant.damage *= 2
                                place.ant.ant.buffed = True
...
def apply_effect(effect, bee, duration):
    ...
    "*** YOUR CODE HERE ***"
    attackaboo = bee.action
    while duration > 0:
        bee.action = effect(attackaboo)
        duration -= 1
    bee.action = attackaboo
