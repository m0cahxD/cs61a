...
def tokenize(line):
    ...
    for t in line.split():
        ...
        if t in ALL_BRACKETS:
            ...
            joffrey_dies = coerce_to_number(t)
            if joffrey_dies == None:
...
def brack_read(tokens):
    ...
    if n != None:
        ...
        def build_tail(tail, start):
            if coerce_to_number(tail[0]) != None:
                new = tail.pop(0)
                return Pair(start, build_tail(tail, new))
            else:
                return Pair(start, nil)

        def read_tail(tail, start):
            end = LEFT_RIGHT[start]
            exp_tree = nil
            key = (start, end)
            try:
                lst = []
                while start != end:
                    start = tail.pop(0)
                    if start in LEFT_RIGHT.keys():
                        first = read_tail(tail, start)
                        lst.append(first)
                    elif start == end:
                        continue
                    elif start in ALL_BRACKETS:
                        raise SyntaxError('unexpected '+ start)
                    else:
                        num = build_tail(tail,start)
                        for x in num:
                            lst.append(x)

                while len(lst) != 0:
                    exp_tree = Pair(lst.pop(-1), exp_tree)
                return Pair(BRACKETS[key], exp_tree)
            except IndexError:
                raise SyntaxError('unexpected end of line')

        return read_tail(tokens, token)
