...
# collaborated with vivian dien, beautiful diety of CS.
...
class VendingMachine(object):
    ...
    def vend(self):
        if self.stock > 0:
            if self.balance > self.price:
                string = 'Here is your ' + str(self.item) + ' and $' + str(self.balance-self.price) + ' change.'
                self.balance = 0
                self.stock -= 1
                return string
            elif self.balance == self.price:
                string = 'Here is your ' + str(self.item) + '.'
                self.balance = 0
                self.stock -= 1
                return string
            else:
                string = 'You must deposit $' + str(self.price - self.balance) + ' more.'
                return string
        else:
    ...
    def deposit(self, amount):
        if self.stock > 0:
            ...
            string = "Machine is out of stock. Here is your $" + str(amount) + '.'
            return string
...
class MissManners(object):
    ...
    def ask(self, *args):
        ...
        if method[0] == 'please':
            string = args[0][7:]
            to_return = getattr(self.parent, string, "Thanks for asking, but I know not how to " + str(string))
            if type(to_return) == str:
                return to_return
            if args[1:] == ():
                return to_return()
            else:
                return to_return(args[1])
        else:
...
class life_lists(life):
    ...
    def _is_alive(self, row, col):
        "*** YOUR CODE HERE ***"
        return life.live_value(self._board[row][col])
    ...
    def tick(self):
        ...
        "*** YOUR CODE HERE ***"
        temp = life_lists(self.rows, self.cols)
        for outer, y in enumerate(self._board):
            for inner, x in enumerate(y):
                next_life = self.survives(outer, inner)
                temp.set_alive(outer, inner, life.live_value(next_life))
        self._board = temp._board
