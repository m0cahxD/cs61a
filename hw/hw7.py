...
def apply(operator_name, shape):
    ...
    apply_implementation = {
        ...
        ('perimeter', Rect): rect_perimeter}
    if type(shape) == Square:
        return apply_implementation[key](shape.side)
    return apply_implementation[key](shape.width, shape.height)
...
def rlist_adjoin(s, v):
    ...
    temp = Rlist.empty
    for item in s:
        temp = drlist_adjoin(temp, item)
    temp = drlist_adjoin(temp, v)
    return temp
...
def rlist_intersect(s1, s2):
    ...
    "*** YOUR CODE HERE ***"
    temp = Rlist.empty
    def intersect_helper(s1, s2):
        nonlocal temp
        if s1 == Rlist.empty or s2 == Rlist.empty:
            return temp
        if s1.first == s2.first:
            temp = drlist_adjoin(temp, s1.first)
            return intersect_helper(s1.rest, s2.rest)
        if s1.first < s2.first:
            return intersect_helper(s1.rest, s2)
        if s1.first > s2.first:
            return intersect_helper(s1, s2.rest)
        print("should never arrive here!")
    return intersect_helper(s1, s2)
...
def rlist_union(s1, s2):
    ...
    "*** YOUR CODE HERE ***"
    temp = Rlist.empty
    def union_helper(s1, s2):
        nonlocal temp
        if s1 == Rlist.empty and s2 == Rlist.empty:
            return temp
        elif s1 == Rlist.empty:
            for item in s2:
                temp = drlist_adjoin(temp, item)
            ...
            temp = drlist_adjoin(temp, s1.first)
            return union_helper(s1.rest, s2.rest)
        elif s1.first < s2.first:
            temp = drlist_adjoin(temp, s1.first)
            return union_helper(s1.rest, s2)
        elif s1.first > s2.first:
            temp = drlist_adjoin(temp, s2.first)
            return union_helper(s1, s2.rest)
        else:
            print("should never reach here!")
    return union_helper(s1, s2)
    ...
    def __next__(self):
        while len(self._work_queue) > 0:
            ...
            if isinstance(subtree_or_label, EmptyBinTree):
                pass
            elif type(subtree_or_label) == int:
                return subtree_or_label
            else:
                self._work_queue += [subtree_or_label.right, subtree_or_label.label, subtree_or_label.left]
...
def partial_sequence_to_tree(s, n):
    ...
    count = n
    while count > 0:
        to_tree.append(s.first)
        s = s.rest
        count -= 1
    def helper(a_list):
        if len(a_list) == 1:
            return BinTree(a_list[0])
        elif len(a_list) == 2:
            return BinTree(a_list[1], BinTree(a_list[0]))
        else:
            size = len(a_list)
            return BinTree(a_list[size//2], helper(a_list[:size//2]), helper(a_list[size//2+1:]))
    return(helper(to_tree), s)
...
def mario_number(level):
    ...
    def luigi(toad):
        ...
        if len(toad) == 2:
            return luigi(toad[1:])
        return luigi(toad[1:]) + luigi(toad[2:])
    return luigi(level)
